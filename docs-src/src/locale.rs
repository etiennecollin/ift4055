...
pub const LOCALE_EN: Locale<'static> = Locale {
    ...
    progress_2_description: "
        To solve the issue of discretization, the actor-critic algorithm was researched, allowing for continuous action spaces by creating a model which returns the probability distribution of the actions from which it is possible to sample an action. A basic implementation of the actor-critic algorithm was created with PQMass as the reward function. I also attended a MILA workshop on RL.
    ...
    progress_3_description: "
        After working on the GFlowNet implementation, it is clear that this architecture is promising for the problem at hand. That being said, the GFlowNet used had trouble finding all of the modes in a sparse distribution. This is probably caused by the usage of an \"on-policy\" model, meaning that the model would sample its next points from the learned policy. This should not be the case in the final project, because the acquisition model will be used to propose the points.

        <br/>
        Up until now, it is therefore clear that a GFlowNet will be used as a surrogate model to emulate/approximate the complex distribution/function. Moreover after discussing with my supervisors, I will be exploring how Gaussian processes and acquisition functions (AF) are used in the context of Bayesian optimization (BO). At first glance, it seems that this project is closely related to BO. In fact, the main difference is that BO only cares about accurately finding the maximal/minimal value of a function (because it is an optimization problem), whereas this project aims to accurately model the entire function and sample from it.

        <br/>
        The next step is to research on papers discussing the use of neural networks as AFs in BO in order to learn how they work and how to adapt them to my problem that is not an optimization problem.
    ",
    ...
    progress_5_description: "
        First, I read on Expectation Maximization, Bayesian inference, Variational Inference and Bayesian Neural Network Regression. These topics are all linked to the idea of adding uncertainty to neural networks. Although I am far from being an expert after a single week, it was extremely interesting how one can program a neural network to inherently output some kind of uncertainty in its predictions.

        <br/>
        Then, after reading papers and looking at how people implemented these kinds of networks, I had another meeting with the PhD student. These meetings have helped me a lot and made me progress. We determined that the entire problem I am facing, that is, the problem of efficiently sampling points in a \"black box\" function to train a surrogate model with the least number of evaluations of the \"black box\", is a problem of active learning. 

        <br/>
        Active learning is the concept of training a model to learn from the data it is given and, in a way, it is to be able to ask for more data where it needs it. Generally, this is used when we have a partially labelled dataset; we can train a model on the labelled data and use that model to label the rest of the dataset. With each prediction, the model also outputs a measure of uncertainty. This uncertainty can be used to determine whether we can use the label to add to the training set or if the sample should be manually labelled. This allows spending less time labelling data that a model can already predict with high confidence.
