...
pub const LOCALE_FR: Locale<'static> = Locale {
    ...
    progress_1_description: "Pour optimiser le processus d'échantillonnage dans le réseau d'apprentissage par renforcement, la fonction de récompense doit quantifier la qualité des points fournis en s'assurant qu'ils représentent la fonction à partir de laquelle ils sont échantillonnés. En d'autres termes, elle doit mesurer la disparité entre la fonction originale et les points échantillonnés générés par le modèle.
